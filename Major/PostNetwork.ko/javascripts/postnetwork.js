$(document).ready(function () {
  ...
  function draw_op_person_after(post,list,name,name_kr){
  ...
  function op_function_change(){
      ...
      if (selected_postposition === "ey"){
          draw_op_person_after("ey",functionslist_ey,functionsname_ey,functionsname_kr_ey)
      } else if (selected_postposition === "eyse"){
          draw_op_person_after("eyse",functionslist_eyse,functionsname_eyse,functionsname_kr_eyse)
      } else if (selected_postposition === "lo"){
          draw_op_person_after("lo",functionslist_lo,functionsname_lo,functionsname_kr_lo)
      }
      drawall();
  }
  ...
  for(var i = 0 ; i < typeslist.length; i++){
      $("#container_leftbottom").append("<input class='CB_leftbottom' type='checkbox' value='"+typeslist[i]+"' id='CB_leftbottom_"+i+"' /> <label class='CB_leftbottom'>"+typeslist[i]+" ("+typesname_kr[i]+", "+typesname[i]+")</label></br>");
  }


  //문장 입력파트(concordance table)
  function drawconcordance_table(data,post,func){
      $('#concordancetable').empty();
      var sentencedata = [];
      for (var i = 0; i < data.length ; i++) {
          if ((data[i].postposition === post) && (data[i].function === func)) {
              sentencedata.push(data[i]);
          }
      }
      for(var i = 0 ; i < sentencedata[0].sentences.length; i++){
          $("#concordancetable").append("<tr style='padding: 0px;'><td>"+(i+1)+"</td><td>"+sentencedata[0].sentences[i].name+"</td><td>"+sentencedata[0].function.toUpperCase()+"</td><td>"+sentencedata[0].sentences[i].sentence+"</td><td>"+sentencedata[0].sentences[i].pos_sentence+"</td></tr>");
      }
  }
  ...
  var svgright_top = d3.select("#right_top")
          ...
          .attr("height", (right_top_height*0.828))
  ...
  function drawall(){
    ...
    drawconcordance_table(sentence_concordance,selected_postposition,selected_function)
  ...
  function firstdrawdata() {
        ...
        for (var i = 0; i < network_map.length ; i++) {
            if ((network_map[i].postposition === 'ey') && (network_map[i].function === 'loc') && (network_map[i].method === 'svd') && (network_map[i].window === 'window1')) {
                data.push(network_map[i]);
            }
        }
        ...
        for (var i = 0; i < network_map.length ; i++) {
            ...
            if ((network_map[i].postposition === 'ey') && (network_map[i].function === 'loc') && (network_map[i].method === 'svd') && (network_map[i].window === 'window1')) {
                for (var j = 0; j < network_map[i].wordnet.length ; j++) {
                    var each = {
                          opacity_value: []
                    };
                    each.opacity_value.push(0.6);
                    var settings = $.extend({}, each, network_map[i].wordnet[j]);
  ...
  function changedrawdata(selected_postposition,selected_function,selected_method,selected_window,selected_node_size,selected_node_color,partofspeeches) {
          ...
          var data = [];
          for (var i = 0; i < network_map.length ; i++) {
              if ((network_map[i].postposition === selected_postposition) && (network_map[i].function === selected_function) && (network_map[i].method === selected_method) && (network_map[i].window === selected_window)) {
                  data.push(network_map[i]);
              }
          }
                  ...
                  for (var j = 0; j < network_map[i].wordnet.length ; j++) {
                      ...
                      if(partofspeeches.length > 0 == true){
                          ...
                          for(var k = 0; k < partofspeeches.length ; k++){
                            if(network_map[i].wordnet[j].pos == partofspeeches[k]){
                                checked = true;
                            }
                          }
                          if(checked == true){
  ...
  function drawtable(data){
          ...
          if(data.links[0].target=="에/ey/JKB"||data.links[0].target=="에서/eyse/JKB"||data.links[0].target=="(으)로/(u)lo/JKB"){
            for(var i = 0 ; i < data.links.length; i++){
  ...
  function drawnetwork(data){
          ...
          var simulation = d3.forceSimulation()
              ...
              .force("center", d3.forceCenter(width * 0.14 / 2, (right_top_height*0.828) / 2));
