$(document).ready(function () {
  ...
  function right_bottom_draw(){
    ...
    //시각화 변경
    function changedrawdata(selected_postposition,functionarray,indexarray) {  //selected_postposition,selected_node_color,functionarray,indexarray
            ...
            var data = {};
            for (var i = 0; i < Map_info.length ; i++) {
                if ((Map_info[i].postposition === selected_postposition) && (Map_info[i].epoch === 'epoch'+EpochNow)) {
                    var innerArray = [];
                    for(var j = 0; j < Sentence_info.length ; j++){
                        if (Sentence_info[j].postposition === selected_postposition && (Map_info[i].epoch === 'epoch'+EpochNow)) {
                            for(var k = 0; k < Map_info[i].sentences.length ; k++){
                                var sentenceDic = {}
                                sentenceDic["index"] = Sentence_info[j].sentences[k].index
                                sentenceDic["function"] = Sentence_info[j].sentences[k].function
